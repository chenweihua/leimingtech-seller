...
public class AllinpayAction {
	...
	/**
	 * 功能：银联页面跳转同步通知页面 版本：3.2 日期：2015-07-02 说明：
	...
	public String FrontRetrun(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		log.debug("===============payfront================");
		// String orderpayCode =
		// Unionpayservice.Unionpayfront(request,response);
		...
		if (payMap.size() != 0) {
			if (payMap.get("payResult").equals("1") && !payMap.get("paymentOrderId").equals("")) {
				payService.updatePayFinish(payMap.get("orderNo").toString(), payMap.get("paymentOrderId").toString(), "pc_allinpay");
...
//	@RequestMapping("/updatePayFinish")
//	public void updatePayFinish(HttpServletRequest request, HttpServletResponse response) {
//		Map<String, Object> payMap = allinPayservice.toPayfront(request, response);
//		payService.updatePayFinish(payMap.get("orderNo").toString(), payMap.get("paymentOrderId").toString(), "pc_allinpay");
//
//	}
	...
	/**
	 * 功能：银联服务器异步通知页面 版本：3.3 日期：2015-07-02 说明：
	 * 以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己网站的需要，按照技术文档编写,并非一定要使用该代码。
	...
	public void BackReturn(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (payMap.size() != 0) {// out_trade_no返回过来的订单号 失败返回failure
										// 成功返回success
				if (payMap.get("payResult").equals("1") && !payMap.get("paymentOrderId").equals("")) {
					payService.updatePayFinish(payMap.get("orderNo").toString(), payMap.get("paymentOrderId").toString(), "pc_allinpay");
				}
