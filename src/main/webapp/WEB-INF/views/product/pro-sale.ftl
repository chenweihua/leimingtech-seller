...
<div class="wrap">
  <div class="tabmenu">
    ...
    <a href="${base}/pro/loadgoodsbystoreid?storeId=${storeId}" class="ncu-btn3" title="导出商品" style="margin-right: 190px;">导出商品</a> 
  </div>
  <form method="get" action="${base}/pro/sale" name="queryListForm">
   ...
   <#if goodsList??>
    <#list goodsList as gl>
      ...
      <tr nc_type="table_item" idvalue="89">
        ...
        <#if gl.createTimeStr?? && gl.createTimeSt!=''>
			<td class="goods-time">${gl.createTimeStr?string('yyyy-MM-dd')}</td>
		</#if>
        
        <#if gl.goodsCommend == 0>
        	<td><span nc_type="editobj" class="tip norecommend-ico-btn" status="off" title="选择是否作为店铺推荐商品"></span></td>
        <#else>
        	<td><span nc_type="editobj" class="tip recommend-ico-btn" status="on" title="选择是否作为店铺推荐商品"></span></td>
        </#if>
        <td class="tc">
