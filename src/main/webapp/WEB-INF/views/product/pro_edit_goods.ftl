...
.box_arr .table_btn a.disable_spec {
	background:
		url(http://192.168.1.220/templates/default/images/member/btn.gif)
		repeat 0 -1018px;
	float: right;
}
					...
					<dl>
						...
						<dd id="gcategory">
							<span class="f_l">${catename}</span> <a
								class="edit_gcategory ncu-btn1 ml10" href="${base}/pro/sell?goodsId=${goodsId}">
								<span>编辑</span>
						...
						<dd nctype="sp_group_val">
							<ul class="spec">
								...
								<li><span nctype="input_checkbox" class="checkbox">
										...
										src="${imageServer}/${svl.spValueImage}"
						...
						<dd>
							...
							<p style="float: left; clear: both; margin-top: 8px;">
								<input type="radio" value="0" name="goods_transfee_charge"
									id="whops_buyer"> <label for="whops_buyer">买家承担运费</label>
							</p>
						...
						<dd>
							...
							<p class="mt5">
								<label> <input name="goods_show" value="0" type="radio" />
										<!-- <#if goods.goodsShow==1>checked="checked"</#if>/> -->
									发布时间
								</label> <input onClick="WdatePicker({minDate:'%y-%M-{%d+1} %H:%m:%s',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true});"
...
var oldspecName = '${goods.specName}';
$(function (){
	// 初始化富文本编辑器
	initKinEdit();
    // 校验表单
